...
class ExcluirFuncionario(private val context: Context) {

    private val auth: FirebaseAuth = FirebaseAuth.getInstance()
    private val db: FirebaseFirestore = FirebaseFirestore.getInstance()

    fun deleteUser(view: View) {
        val userId = auth.currentUser?.uid // Obtenha o ID do usuário atual
        if (userId != null) {
            ...
            alertDialog.setPositiveButton("Sim") { dialog: DialogInterface, _: Int ->
                ...
                val userRef = db.collection("profissionais").document(userId)
